...
void LambertBlinnDecalRendererTests::update(ELFloat deltaTime) {
    ...
    ELMatrix4 decalProjection = ELMatrix4MakeOrtho(-0.2, 0.2, 0.2, -0.2, -100, 100);
    ELMatrix4 decalView = ELMatrix4MakeLookAt(0, 0, 1.1, 0, 0, 0, 0, 1, 0);


    lambertBlinnDecalRenderer->beginDraw();

    lambertBlinnDecalRenderer->setDecalMap(decalTexture);
    lambertBlinnDecalRenderer->setDecalProjectionMatrix(ELMatrix4Multiply(decalProjection, decalView));
    lambertBlinnDecalRenderer->setDecalAlpha(1.0);

    lambertBlinnDecalRenderer->setVertexBuffer(sphereVertexBuffer);
    lambertBlinnDecalRenderer->setLights(light, sizeof(light) / sizeof(ELLightInfo));
    lambertBlinnDecalRenderer->setCameraPosition(ELVector3Make(0, 0, 1.1));
    lambertBlinnDecalRenderer->setMaterial(material);
    lambertBlinnDecalRenderer->setDiffuseMap(diffuseTexture);
    lambertBlinnDecalRenderer->setModelMatrix(model);
    lambertBlinnDecalRenderer->setNormalMatrix(normalMatrix);
    lambertBlinnDecalRenderer->setViewMatrix(view);
    lambertBlinnDecalRenderer->setProjectionMatrix(projection);
    lambertBlinnDecalRenderer->draw(ELPrimitivesTypeTriangle);

    lambertBlinnDecalRenderer->commitDraw();
}

LambertBlinnDecalRendererTests::LambertBlinnDecalRendererTests(std::map<std::string, ELRenderPiplinePtr> piplines) {
    ...
    lambertBlinnDecalRenderer = ELLambertBlinnDecalRenderer::alloc()->init(config, defaultRenderTarget);


    std::string imagePath = ELAssets::shared()->findFile("texture.jpg");
    diffuseTexture = ELTexture::alloc()->init(imagePath);

    std::string decalImagePath = ELAssets::shared()->findFile("grass_02.jpg");
    decalTexture = ELTexture::alloc()->init(decalImagePath);
